                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center space-x-1">
                                ...
                                @if($budget->status === 'draft')
                                    ...
                                    <form id="delete-form-budget-{{ $budget->id }}" action="{{ route('budget.destroy', $budget) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDeleteModal('delete-form-budget-{{ $budget->id }}', 'Yakin ingin menghapus anggaran {{ $budget->name }}?')" 
