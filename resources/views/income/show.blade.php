        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-soft border border-gray-100/50 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <form id="delete-form-income-{{ $income->id }}" action="{{ route('income.destroy', $income) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDeleteModal('delete-form-income-{{ $income->id }}', 'Yakin ingin menghapus transaksi pemasukan {{ $income->transaction_number }}?')" 
