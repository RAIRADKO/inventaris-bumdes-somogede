                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center space-x-1">
                                ...
                                @if($journal->status === 'draft')
                                    ...
                                    <form id="delete-form-journal-{{ $journal->id }}" action="{{ route('journal.destroy', $journal) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDeleteModal('delete-form-journal-{{ $journal->id }}', 'Yakin ingin menghapus jurnal {{ $journal->journal_number }}?')" 
