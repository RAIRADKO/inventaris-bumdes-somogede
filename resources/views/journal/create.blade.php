...
@section('subtitle', 'Catat transaksi jurnal baru')
    ...
    <div class="bg-white rounded-2xl shadow-soft overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-indigo-50">
            <div class="flex items-center">
                ...
                <div>
                    <h3 class="font-semibold text-gray-800">Buat Jurnal Baru</h3>
                    <p class="text-sm text-gray-500">Isikan entri debit dan kredit</p>
                </div>
            </div>
        </div>

        <form action="{{ route('journal.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" required
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition @error('date') border-rose-500 @enderror">
                    @error('date')
                ...
                <div>
                    ...
                    <select name="business_unit_id" id="business_unit_id" required
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition @error('business_unit_id') border-rose-500 @enderror">
                ...
                {{-- Fiscal Period (read-only info) --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Periode Fiskal</label>
                    <div class="px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-600">
                        {{ $fiscalPeriod?->name ?? 'Tidak ada periode aktif' }}
                    </div>
                </div>
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="description" id="description" rows="2" required
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition resize-none @error('description') border-rose-500 @enderror"
                              placeholder="Keterangan transaksi jurnal...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
