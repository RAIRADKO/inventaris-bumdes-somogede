...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition">
        ...
        <div class="flex gap-2 mt-4">
            ...
            <form action="{{ route('business-unit.destroy', $unit) }}" method="POST" class="inline" 
                  onsubmit="return confirm('Yakin ingin menghapus unit usaha ini?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-3 py-2 text-rose-500 hover:bg-rose-50 text-sm rounded-lg transition" title="Hapus">
