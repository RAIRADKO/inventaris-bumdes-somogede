...
@section('subtitle', $expense->transaction_number)

@section('content')
<div class="max-w-4xl mx-auto">
    <!-- Back Button -->
    <a href="{{ route('expense.index') }}" class="inline-flex items-center text-gray-400 hover:text-gray-600 mb-6 group">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-soft border border-gray-100/50 overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-rose-50 to-white">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <div class="w-14 h-14 bg-gradient-to-br from-rose-400 to-rose-600 rounded-xl flex items-center justify-center mr-4 shadow-lg shadow-rose-200/50">
                                <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                </svg>
                            </div>
                            <div>
                                <h2 class="text-xl font-bold text-gray-800">{{ $expense->transaction_number }}</h2>
                                <p class="text-sm text-gray-400">{{ $expense->date->format('d F Y') }}</p>
                            </div>
                        </div>
                        @php
                            $statusColors = [
                        ...
                        @endphp
                        <span class="px-4 py-2 {{ $statusColors[$expense->status] }} text-sm font-bold rounded-xl uppercase">
                ...
                <div class="p-6">
                    <!-- Amount -->
                    <div class="mb-6 p-4 bg-gradient-to-r from-rose-50 to-red-50 rounded-xl">
                        <p class="text-sm text-gray-500 mb-1">Jumlah Pengeluaran</p>
                        <p class="text-3xl font-bold text-rose-600">Rp {{ number_format($expense->amount, 0, ',', '.') }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="font-semibold text-gray-800">{{ $expense->category->name ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-400 mb-1">Unit Usaha</p>
                            <p class="font-semibold text-gray-800">{{ $expense->businessUnit->name ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-400 mb-1">Penerima</p>
                            <p class="font-semibold text-gray-800">{{ $expense->recipient ?? '-' }}</p>
                        </div>
                        <div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-soft border border-gray-100/50 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <form action="{{ route('expense.submit', $expense) }}" method="POST">
                        @csrf
                    ...
                    <form id="delete-form-expense-{{ $expense->id }}" action="{{ route('expense.destroy', $expense) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDeleteModal('delete-form-expense-{{ $expense->id }}', 'Yakin ingin menghapus transaksi pengeluaran {{ $expense->transaction_number }}?')" 
                    ...
                    @if($expense->status === 'pending' && auth()->user()->canApprove())
                    <form action="{{ route('expense.approve', $expense) }}" method="POST">
                        @csrf
                    ...
                    <form action="{{ route('expense.reject', $expense) }}" method="POST">
                        @csrf
            ...
            <div class="bg-white rounded-2xl shadow-soft border border-gray-100/50 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="font-medium text-gray-700">{{ $expense->createdBy->name ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-1">Tanggal Input</p>
                        <p class="font-medium text-gray-700">{{ $expense->created_at->format('d M Y, H:i') }}</p>
                    </div>
                    @if($expense->approved_at)
                    <div>
                        <p class="text-xs text-gray-400 mb-1">Disetujui oleh</p>
                        <p class="font-medium text-gray-700">{{ $expense->approvedBy->name ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-1">Tanggal Disetujui</p>
                        <p class="font-medium text-gray-700">{{ $expense->approved_at->format('d M Y, H:i') }}</p>
                    </div>
                    @endif
                </div>
            </div>
